...
| **Project slug** | a slug-case name and compact prefix drawn from the repo name (e.g. `echo-vault` / `ev`), recorded as a `## Project slug` block for skills that file per-project output (currently `gpt-image-2`) | always — two quick questions, each with an acceptable-in-a-word default |
