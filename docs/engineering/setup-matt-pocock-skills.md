...
- **Project slug** — two questions, a slug-case project name and a compact prefix (e.g. `echo-vault` / `ev`), recorded as a `## Project slug` block for any skill that files per-project output (currently `gpt-image-2`).
